...
pub fn json_to_value(j: &serde_json::Value) -> Option<Value> {
    match j {
...
pub fn state_to_json(state: &State, vars: &[Arc<str>]) -> serde_json::Value {
...
pub fn json_to_state(j: &serde_json::Value, vars: &[Arc<str>]) -> Option<State> {
    let obj = j.as_object()?;
