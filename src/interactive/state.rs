...
use crate::ast::{Env, Spec, State, Transition, Value, VarChange, TransitionWithGuards};
use crate::checker::format_value;
use crate::eval::{eval, make_primed_names, next_states, next_states_with_guards, Definitions};

use super::serialize::{state_to_json, json_to_state};
...
impl ExplorerState {
    pub(crate) fn new(initial: State, actions: Vec<Transition>, actions_with_guards: Vec<TransitionWithGuards>) -> Self {
        Self {
            current: initial,
            history: Vec::new(),
    ...
    pub(crate) fn take_action_at(&mut self, action_idx: usize, spec: &Spec, env: &mut Env, defs: &Definitions) {
    ...
    fn refresh_actions(&mut self, spec: &Spec, env: &mut Env, defs: &Definitions) {
        ...
        match next_states(&spec.next, &self.current, &spec.vars, &primed_vars, env, defs) {
            Ok(actions) => {
                self.available_actions = actions.clone();
                match next_states_with_guards(&spec.next, &self.current, &spec.vars, &primed_vars, env, defs) {
                    Ok(guards) => self.available_actions_with_guards = guards,
                    Err(_) => self.available_actions_with_guards = actions.iter().map(|t| TransitionWithGuards {
                        transition: t.clone(),
                        guards: Vec::new(),
                ...
                for (k, v) in new_map {
                    ...
                    match old_map.get(k) {
                        Some(old_v) if old_v != v => {
                            self.record_value_changes(state_idx, new_path, old_v, v, action.clone());
                        }
                        None => {
    ...
    pub(crate) fn reset(&mut self, initial: State, actions: Vec<Transition>, actions_with_guards: Vec<TransitionWithGuards>) {
    ...
    pub(crate) fn random_walk(&mut self, steps: usize, spec: &Spec, env: &mut Env, defs: &Definitions) {
        for i in 0..steps {
            if self.available_actions.is_empty() {
    ...
    pub(crate) fn step_until(
        ...
        self.status_message = Some((
            format!("Condition not met after {} steps", max_steps),
            true,
        ));
    }

    pub(crate) fn save_trace(&self, path: &Path, vars: &[Arc<str>], initial: &State) -> io::Result<()> {
    ...
    pub(crate) fn load_trace(
        ...
        let primed_vars = make_primed_names(&spec.vars);
        let available_actions = next_states(&spec.next, &current, &spec.vars, &primed_vars, env, defs)
            .map_err(|e| io::Error::other(format!("{:?}", e)))?;
        let available_actions_with_guards = next_states_with_guards(&spec.next, &current, &spec.vars, &primed_vars, env, defs)
            .unwrap_or_else(|_| available_actions.iter().map(|t| TransitionWithGuards {
                transition: t.clone(),
                guards: Vec::new(),
                parameter_bindings: Vec::new(),
            }).collect());
