...
pub(super) fn run_app(
    ...
    defs: &Definitions,
    initial: &State,
    initial_actions: &[Transition],
    initial_actions_with_guards: &[TransitionWithGuards],
) -> io::Result<()> {
    loop {
        terminal.draw(|f| ui(f, explorer, spec, env, defs))?;
            ...
            match explorer.input_mode {
                InputMode::Repl => {
                    match key.code {
                        ...
                        KeyCode::Enter => {
                            let result = eval_repl(&explorer.repl_input, &explorer.current, spec, env, defs);
                ...
                InputMode::Hypothesis => {
                    match key.code {
                        ...
                        KeyCode::Enter => {
                            let result = test_hypothesis(&explorer.hypothesis_input, &explorer.history, &explorer.current, spec, env, defs);
                    ...
                    match key.code {
                        ...
                        KeyCode::Enter => {
                            ...
                            match explorer.walk_input.trim().parse::<usize>() {
                                ...
                                Ok(n) => {
                                    explorer.random_walk(n, spec, env, defs);
                                }
                                Err(_) => {
                    ...
                    match key.code {
                        ...
                        KeyCode::Enter => {
                            ...
                            match crate::parser::parse_expr(expr_str) {
                                Ok(condition) => {
                                    explorer.step_until(&condition, max_steps, spec, env, defs);
                                }
                                Err(e) => {
                                    explorer.status_message = Some((format!("Parse error: {}", e.message), true));
                                }
                            }
                        }
                        KeyCode::Backspace => {
                ...
                InputMode::Normal => {
                    match key.code {
                        ...
                        KeyCode::Enter => explorer.take_action(spec, env, defs),
                        KeyCode::Char('b') => explorer.backtrack(spec, env, defs),
                        KeyCode::Char('r') => {
                            explorer.reset(initial.clone(), initial_actions.to_vec(), initial_actions_with_guards.to_vec());
                        }
                        KeyCode::Char('e') => {
                        ...
                        KeyCode::Char('g') => {
                            ...
                            let status = if explorer.show_guards { "Guards ON" } else { "Guards OFF" };
                        ...
                        KeyCode::Char('s') => {
                            ...
                            match explorer.save_trace(path, &spec.vars, initial) {
                                Ok(()) => {
                                    explorer.status_message = Some((format!("Saved trace to {}", path.display()), false));
                                }
                                Err(e) => {
                                    explorer.status_message = Some((format!("Save failed: {}", e), true));
                                }
                            }
                        }
                        KeyCode::Char('l') => {
                            let path = Path::new("trace.json");
                            match ExplorerState::load_trace(path, spec, env, defs, initial) {
                                Ok(loaded) => {
                                    *explorer = loaded;
                                    explorer.status_message = Some((format!("Loaded trace from {}", path.display()), false));
                                }
                                Err(e) => {
                                    explorer.status_message = Some((format!("Load failed: {}", e), true));
                                }
                            }
                        }
                        _ => {}
            ...
            match key.code {
                ...
                KeyCode::Char('n') | KeyCode::Right | KeyCode::Down => {
                    if explorer.replay_position < explorer.replay_trace.len() - 1 {
                        ...
                        explorer.current = explorer.replay_trace[explorer.replay_position].0.clone();
                        let action = &explorer.replay_trace[explorer.replay_position].1;
                        explorer.status_message = Some((
                            format!("Step {}/{} - {}", explorer.replay_position, explorer.replay_trace.len() - 1, action.as_ref().map(|a| a.to_string()).unwrap_or_else(|| "Init".to_string())),
                            false,
                        ));
                    } else {
