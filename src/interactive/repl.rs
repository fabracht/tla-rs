...
use crate::eval::{eval, make_primed_names, Definitions};
use crate::ast::Env;

pub(super) fn eval_repl(input: &str, current: &State, spec: &Spec, env: &Env, defs: &Definitions) -> String {
...
pub(super) fn test_hypothesis(
    ...
    for (i, (next_state, action)) in history.iter().skip(1).chain(std::iter::once(&(current.clone(), None))).enumerate() {
        let mut eval_env = env.clone();
        ...
        match eval(&expr, &mut eval_env, defs) {
            Ok(Value::Bool(true)) => {}
            Ok(Value::Bool(false)) => {
                let action_name = action.as_ref().map(|a| a.to_string()).unwrap_or_else(|| "(unnamed)".to_string());
    ...
    if blocked_transitions.is_empty() {
        format!("Hypothesis '{}' would NOT have blocked any transitions", guard_expr)
    } else {
        let mut result = format!("Hypothesis '{}' would have blocked {} transition(s):\n", guard_expr, blocked_transitions.len());
        for (i, action) in blocked_transitions.iter().take(5) {
            result.push_str(&format!("  State {} -> {}: {} would be BLOCKED\n", i, i + 1, action));
        }
