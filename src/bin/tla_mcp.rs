...
impl ServerHandler for TlaMcpServer {
    fn get_info(&self) -> ServerInfo {
        ...
        info.instructions = Some(
            ...
            \n\
            • Demos read best on named-action specs. validate_demo / append_beat / export_demo_* pin each scenario step with `action: <Name>` — one self-describing step per move, and one hotkeyed explorer row per action. A spec whose `Next` is a single `\\E ...` quantifier has no action to name, so steps fall back to `step:` predicates that re-state the next state and every transition lands under one unnamed group — it still works, just reads worse. A named action that itself quantifies over a domain (`Reboot == \\E v \\in 0..MaxTime : wallClock' = v`) is fine: the explorer factors its variants into the shared effect plus one value picker per differing variable, so it stays compact even for large domains. When authoring a demo, prefer lifting common moves into named operators over adding a synthetic tag variable, and expose the key property as a named definition (e.g. `Solved == ...`) so beats can assert `final: Solved` and the explorer shows it live."
                .to_string(),
        );
        info
    }
