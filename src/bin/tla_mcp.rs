...
impl TlaMcpServer {
    #[tool(
        description = "Parse a TLA+ spec, return a structured summary (vars, detected invariants, init/next presence, definition_count) or a parse/config error with source span. Call this after EVERY edit to a .tla file. Reality check: the parser silently skips operator bodies it can't parse, so an invariant whose body has a typo will simply not appear in `spec.invariants` — and `check_spec` will then 'pass' without ever checking it. If an invariant name you expected is missing, the operator's body has an error or the name does not match auto-detection (Inv*, TypeOK*, NotSolved*)."
    )]
    ...
    #[tool(
        description = "Run the TLA+ model checker. REQUIRED: max_states and max_depth (no defaults — budget upfront). Optional booleans `allow_deadlock` and `check_liveness` default to the cfg's setting; omit to defer, pass true/false to override. Returns one of:\n\
        ...
        - status='limit_reached': budget exhausted with `limit` ('max_states' or 'max_depth'). NOT a pass; inconclusive. Grow the budget if state space looks legitimate, otherwise shrink it (smaller constants, or `symmetry` for interchangeable model values).\n\
        ...
        Start small: max_states=10000, max_depth=50, smallest non-trivial constants (Procs='{p1,p2}'). Most algorithmic bugs surface at 2-3 instances; large constants are for confidence, not discovery."
    )]
    async fn check_spec(
        &self,
